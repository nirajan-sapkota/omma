...
<title>@yield('title', 'Omma Health Center')</title>
  ...
  <div class="topbar-inner">
    ...
    <nav class="mainnav" id="mainnav">
      ...
      @auth
        ...
        <a href="{{ route('booking.create') }}" class="{{ request()->is('book') ? 'active' : '' }}">Book now</a>
      @else
    ...
    <div class="patient-chip">
      ...
      @else
        <a href="{{ route('login') }}" class="btn btn-ghost btn-sm" style="padding:2px 4px;">Sign in</a>
      @endauth
    </div>
  </div>
</div>
